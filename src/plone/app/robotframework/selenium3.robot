...
...                        run_on_failure=${SELENIUMLIBRARY_RUN_ON_FAILURE}
...
${SELENIUMLIBRARY_RUN_ON_FAILURE}  No operation
...
Element should not remain visible
    [Documentation]  Due to the internals of SeleniumLibrary, a disappearing
...
Element should become visible
    [Documentation]  Due to the internals of SeleniumLibrary, an appearing
    ...              element may rise StaleElementReferenceException (element
    ...              disappears between it's located and inspected). This
    ...              keyword is a workaround that tries again until success.
    [Arguments]  ${locator}
